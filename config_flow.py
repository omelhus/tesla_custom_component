...
import httpx
from .teslajsonpy import Controller as TeslaAPI, Connection as TeslaConnection, TeslaException
        ...
        return vol.Schema(
            {
                ...
                vol.Optional(CONF_TOKEN, msg="Refresh Token"): str,
            }
        )
...
async def validate_input(hass: core.HomeAssistant, data):
    ...
    try:
        controller = TeslaAPI(
            ...
            password=(data[CONF_PASSWORD] if CONF_PASSWORD in data and CONF_TOKEN not in data else None),
            update_interval=DEFAULT_SCAN_INTERVAL,
            refresh_token=(data[CONF_TOKEN] if CONF_TOKEN in data else None)
        )
        ...
        config[CONF_PASSWORD] = (data[CONF_PASSWORD] if CONF_PASSWORD in data and CONF_TOKEN not in data else None)
        ...
        if ex.code == HTTP_UNAUTHORIZED:
            ...
            raise InvalidAuth() from ex
        _LOGGER.error("Unable to communicate with Tesla API: %s. %s was %s", ex.message, CONF_TOKEN, data[CONF_TOKEN] if CONF_TOKEN in data else "None")
